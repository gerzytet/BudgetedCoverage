...
for suffix in ['CLUSTERED', 'RANDOM', 'EXPONENTIAL']:
	name = f'experiment3_budget_{suffix}'
	data = open(f"{name}.csv", "r").readlines()
	header = data[0].strip()
	data = data[1:]
	split_data = []
	algs = set()
	...
	for a in sorted(list(algs)):
		for data in header_parts[2:]:
			new_header_list.append(a + " " + data)
	header = ','.join(new_header_list)

	with open(f"{name}_processed.csv", "w") as file:
		file.write(header + '\n')
		for budget in budgets:
			row = []
			row.append(budget)
			for alg in sorted(list(algs)):
